...
@app.command()
def to_json(src_directory: str = TSP_SRC_DIR, out_directory: str = TSP_OUT_DIR):
    ...
    """
    print(f"[bold yellow]Started converting TSP instances in {src_directory} to JSON...[/bold yellow]")
    ...
    for tsp_inst_filename in tsp_inst_filenames:
        try:
            instance_to_json(tsp_inst_filename, best_cost_by_inst, out_directory)
        except Exception as e:
            print(f"[red]Failed to convert TSP instance {tsp_inst_filename} with error {e}[/red]")
    
    print(f"[bold yellow]Done converting TSP instances in '{src_directory}' to JSON, results written in directory '{out_directory}'[/bold yellow]")
...
def instance_to_json(tsp_inst_filename: str, best_cost_by_inst: Dict[str, float], out_directory) -> None:
    ...
    tsp_dict["best_known_cost"] = best_cost_by_inst.get(tsp_instance.name, UNKNOWN_BEST_COST)
    ...
    print(f"[cyan]Done converting TSP instance {tsp_inst_name},  result saved in {out_filepath}[/cyan]")
...
def collect_distances_matrix(tsp_instance) -> List[List[float]]:
    ...
    """
    print(f"[bold]collecting distances for TSP instance {tsp_instance.name}, dimension={tsp_instance.dimension}, distance type = {tsp_instance.edge_weight_type}[/bold]")
